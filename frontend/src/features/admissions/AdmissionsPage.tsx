...
export const AdmissionsPage: React.FC = () => {
  ...
  const [form, setForm] = useState({ patientId: '', attendingDoctorId: doctorId || '', roomNumber: '', bedNumber: '', reason: '', initialPayment: 0 });
  const resetForm = () => setForm({ patientId: '', attendingDoctorId: doctorId || '', roomNumber: '', bedNumber: '', reason: '', initialPayment: 0 });
  const refresh = () => {
  ...
  const admitMutation = useMutation({
    mutationFn: async () => {
      const { initialPayment, ...fields } = form;
      return (await apiClient.post('/admissions', { ...fields, totalAmount: initialPayment })).data.data;
    },
    onSuccess: () => { refresh(); setIsAdmitOpen(false); resetForm(); setFeedback({ type: 'success', text: 'Patient admitted successfully.' }); },
    onError: (error) => setFeedback({ type: 'error', text: getApiErrorMessage(error, 'Unable to admit patient. Please try again.') }),
  });
  const paymentMutation = useMutation({
    ...
    onSuccess: (data) => { refresh(); setSelectedAdmission(data); setFeedback({ type: 'success', text: `IPD payment of ₹${Number(paymentAmount).toLocaleString()} recorded for ${data.patientName}.` }); setPaymentAmount(''); },
    onError: (error) => setFeedback({ type: 'error', text: getApiErrorMessage(error, 'Unable to record IPD payment. Please try again.') }),
  });
  const dischargeMutation = useMutation({
    mutationFn: async () => (await apiClient.post(`/admissions/${selectedAdmission.id}/discharge`, { dischargeSummary: dischargeSummary.trim() })).data.data,
    onSuccess: () => { refresh(); setSelectedAdmission(null); setDischargeSummary(''); setFeedback({ type: 'success', text: 'Discharge completed. We wish the patient a healthy recovery.' }); },
    onError: (error) => setFeedback({ type: 'error', text: getApiErrorMessage(error, 'Unable to discharge patient. Please try again.') }),
  });

  return <div className="space-y-6">
    {feedback && <div role="alert" className={`rounded-xl border px-4 py-3 text-sm font-medium ${feedback.type === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-rose-200 bg-rose-50 text-rose-800'}`}>{feedback.text}</div>}
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="flex items-center gap-2 text-2xl font-black text-slate-900"><Bed className="h-6 w-6 text-brand-600" /> IPD Admissions</h1><p className="mt-1 text-sm text-slate-500">Track admitted patients, rounds, interim payments, and discharge settlement.</p></div><Button variant="primary" leftIcon={<ClipboardPlus className="h-4 w-4" />} onClick={() => { resetForm(); setIsAdmitOpen(true); }}>Admit Patient</Button></div>
    ...
    <Modal isOpen={isAdmitOpen} onClose={() => setIsAdmitOpen(false)} title="Admit Patient" description="Create an active inpatient admission."><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); admitMutation.mutate(); }}><Input label="Admission Date" type="date" value={admissionDate()} readOnly helperText="Admission time is recorded automatically." /><label className="block text-sm font-semibold">Patient<select className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm" required value={form.patientId} onChange={(e) => setForm({ ...form, patientId: e.target.value })}><option value="">Select patient</option>{patients.map((patient: any) => <option key={patient.id} value={patient.id}>{patient.fullName} · {patient.mobile}</option>)}</select></label><label className="block text-sm font-semibold">Attending doctor<select className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm" value={form.attendingDoctorId} onChange={(e) => setForm({ ...form, attendingDoctorId: e.target.value })}><option value="">Select doctor</option>{doctors.map((doctor: any) => <option key={doctor.id} value={doctor.id}>{doctor.name}</option>)}</select></label><div className="grid grid-cols-2 gap-3"><Input label="Room" value={form.roomNumber} onChange={(e) => setForm({ ...form, roomNumber: e.target.value })} /><Input label="Bed" value={form.bedNumber} onChange={(e) => setForm({ ...form, bedNumber: e.target.value })} /></div><Input label="Admission reason" value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} required /><Input label="Initial payment made (₹)" type="number" min="0" value={form.initialPayment} onChange={(e) => setForm({ ...form, initialPayment: Number(e.target.value) || 0 })} /><div className="flex justify-end gap-3"><Button type="button" variant="secondary" onClick={() => setIsAdmitOpen(false)}>Cancel</Button><Button type="submit" variant="primary" isLoading={admitMutation.isPending}>Admit Patient</Button></div></form></Modal>
    <Modal isOpen={Boolean(selectedAdmission)} onClose={() => setSelectedAdmission(null)} title={selectedAdmission ? `${selectedAdmission.patientName} · ${selectedAdmission.admissionNumber}` : 'Admission'} description="Record an IPD payment or complete the discharge process.">{selectedAdmission && <div className="space-y-5"><div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><h3 className="text-sm font-bold text-slate-900">Collection Summary</h3><div className="mt-3 grid grid-cols-3 gap-3 text-sm"><div><span className="block text-xs text-slate-500">Total collection</span><strong>₹{Number(selectedAdmission.totalAmount).toLocaleString()}</strong></div><div><span className="block text-xs text-slate-500">Received</span><strong className="text-emerald-700">₹{Number(selectedAdmission.paidAmount).toLocaleString()}</strong></div><div><span className="block text-xs text-slate-500">Outstanding</span><strong className="text-rose-700">₹{Number(selectedAdmission.pendingAmount).toLocaleString()}</strong></div></div></div><div className="grid grid-cols-2 gap-3"><Input label="Interim payment (₹)" type="number" min="1" value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} /><div className="flex items-end"><Button className="w-full" variant="outline" onClick={() => paymentMutation.mutate()} isLoading={paymentMutation.isPending} disabled={!paymentAmount}>Record Payment</Button></div></div><label className="block text-sm font-semibold">Discharge summary<textarea required minLength={2} value={dischargeSummary} onChange={(e) => setDischargeSummary(e.target.value)} className="mt-1.5 min-h-24 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm" /></label><Button className="w-full" variant="primary" onClick={() => dischargeMutation.mutate()} isLoading={dischargeMutation.isPending} disabled={dischargeSummary.trim().length < 2} leftIcon={<LogOut className="h-4 w-4" />}>Discharge Patient</Button></div>}</Modal>
  </div>;
};
