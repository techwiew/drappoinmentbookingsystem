# ClinicFlow — Deployment Guide for MilesWeb / VPS Server

This document outlines the step-by-step production deployment process for hosting ClinicFlow on a standard Linux VPS or MilesWeb hosting environment without requiring containerization.
...
- **Domain mapping**:
  - `app.yourdomain.com` $\rightarrow$ React SPA Static Files (`/var/www/clinicflow/frontend/dist`)
...
Create `/var/www/clinicflow/backend/.env`:
```env
NODE_ENV=production
PORT=5000
DATABASE_URL="mysql://clinicuser:SecurePass123!@localhost:3306/clinicflow_db"
...
Create `/var/www/clinicflow/frontend/.env.production`:
```env
...
```bash
cd /var/www/clinicflow/frontend
npm install
npm run build
```
...
```bash
cd /var/www/clinicflow/backend
pm2 start dist/server.js --name "clinicflow-api"
pm2 save
pm2 startup
```
