# MediNovel — Complete User & Developer Setup Guide

> **MediNovel** is a subscription-based multi-tenant doctor clinic management SaaS built with React 18 + TypeScript (Frontend), Node.js + Express + Prisma (Backend), and SQLite (local) / MySQL (production).
...
# DATABASE_URL="mysql://USER:PASSWORD@HOST:3306/MediNovel_db"

# ── JWT Secrets (change these in production!) ─────────────────
JWT_ACCESS_SECRET="MediNovel_access_secret_change_in_production"
JWT_REFRESH_SECRET="MediNovel_refresh_secret_change_in_production"
...
```sql
CREATE DATABASE MediNovel_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'MediNovel_user'@'%' IDENTIFIED BY 'StrongPassword@123';
GRANT ALL PRIVILEGES ON MediNovel_db.* TO 'MediNovel_user'@'%';
FLUSH PRIVILEGES;
```
...
Update `backend/.env`:
```env
DATABASE_URL="mysql://MediNovel_user:StrongPassword@123@your-mysql-host:3306/MediNovel_db"
```
...
✅ Super Admin created: admin@MediNovel.com
...
Expected output:
```
🏥 MediNovel API Server running at http://localhost:5000
...
```
[backend]  🏥 MediNovel API Server running at http://localhost:5000
...
| :--- | :--- | :--- | :--- |
| 👑 **Super Admin** | `admin@MediNovel.com` | `Admin@123` | `/super-admin` Platform Console |
| 👑 **Real Super Admin** | `superadmin@MediNovel.com` | `SuperAdmin@123` | `/super-admin` Platform Console |
